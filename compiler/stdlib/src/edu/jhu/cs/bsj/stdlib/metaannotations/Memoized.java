...
/**
 * This meta-annotation metaprogram adds memoization for a method. This is performed by creating a private class which
 * serves as a tuple to encapsulate the method's parameters. This tuple is used as a key for a map which stores previous
 * results of the method. The original method is renamed and put in its place is a wrapper method which performed
 * memoization checking before calling out to the original method.
 * 
 * @author Joseph Riley
 */
public class Memoized extends AbstractBsjMetaprogramMetaAnnotation
{
	...
	private MethodDeclarationNode generateNewMethod(MethodDeclarationNode method, BsjNodeFactory factory)
	{
		...
		return factory.makeMethodDeclarationNode(method.getBody().deepCopy(factory), modifiers,
				factory.makeIdentifierNode(newMethodName), method.getParameters().deepCopy(factory),
				method.getReturnType().deepCopy(factory), method.getJavadoc() == null ? null
						: method.getJavadoc().deepCopy(factory));
	}
	...
	private void addMemoizationCode(MethodDeclarationNode method, BsjNodeFactory factory)
	{
		...
		String tupleInstanceName = Character.toLowerCase(tupleClassName.charAt(0)) + tupleClassName.substring(1)
				+ "Instance";

		// use parameters for arguments in tuple constructor
		for (VariableNode variable : method.getParameters())
		{
			arguments.add(factory.makeVariableAccessNode(null, variable.getIdentifier().deepCopy(factory)));
		}
	...
	private ClassDeclarationNode generateTupleClassDeclaration(MethodDeclarationNode method, BsjNodeFactory factory)
	{
		...
		for (VariableNode param : method.getParameters())
		{
			...
			members.add(factory.makeFieldDeclarationNode(factory.makeFieldModifiersNode(AccessModifier.PRIVATE, false,
					false, false, false, factory.makeMetaAnnotationListNode(metaAnnotation),
					factory.makeAnnotationListNode()), param.getType().deepCopy(factory),
					factory.makeVariableDeclaratorListNode(factory.makeVariableDeclaratorNode(
							param.getIdentifier().deepCopy(factory), null)), null));
		}
		...
		tupleMetaAnnotations.add(factory.makeNormalMetaAnnotationNode(factory.makeMetaAnnotationElementListNode(),
		...
		return factory.makeClassDeclarationNode(
				factory.makeClassModifiersNode(AccessModifier.PRIVATE, false, true, false, false,
						factory.makeMetaAnnotationListNode(tupleMetaAnnotations), factory.makeAnnotationListNode()),
				null, factory.makeDeclaredTypeListNode(), factory.makeClassBodyNode(members),
				factory.makeTypeParameterListNode(), factory.makeIdentifierNode(tupleClassName),
				factory.makeJavadocNode("Tuple class for storing memoized parameters for method " + methodName + "."));
	}

	private FieldDeclarationNode generateHashMapDeclaration(MethodDeclarationNode method, BsjNodeFactory factory)
	{
		...
		return factory.makeFieldDeclarationNode(factory.makeFieldModifiersNode(AccessModifier.PRIVATE),
				factory.makeParameterizedTypeNode(
						factory.makeUnparameterizedTypeNode(factory.parseNameNode("java.util.Map")),
						factory.makeTypeArgumentListNode(paramType, returnType)),
				factory.makeVariableDeclaratorListNode(factory.makeVariableDeclaratorNode(
						factory.makeIdentifierNode(hashMapName),
						factory.makeUnqualifiedClassInstantiationNode(factory.makeParameterizedTypeNode(
								factory.makeUnparameterizedTypeNode(factory.parseNameNode("java.util.HashMap")),
								factory.makeTypeArgumentListNode(paramType.deepCopy(factory),
										returnType.deepCopy(factory)))))),
