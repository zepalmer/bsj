...
/**
 * This meta-annotation metaprogram generates the toString method for a class. It generates this function based on the
 * getters for properties which exist on that class; that is, all methods of the form <code>getFoo()</code> where
 * <code>Foo</code> is an arbitrary name are invoked by the generated methods.
 * <p/>
...
public class GenerateToString extends AbstractPropertyListMetaannotationMetaprogram
{
			...
			List<Pair<String, TypeNode>> getterDescriptions)
	{
		...
		members.addLast(createToString(context, getterDescriptions, TypeDeclUtils.getEnclosingTypeName(context, this)));
	}
			...
			List<Pair<String, TypeNode>> getters, IdentifierNode classIdentifier)
	{
		...
		statements.add(factory.makeLocalVariableDeclarationNode(
				factory.makeUnparameterizedTypeNode(factory.makeSimpleNameNode(factory.makeIdentifierNode("String"))),
		...
		for (Pair<String, TypeNode> getter : getters)
		{
			...
			statements.add(factory.makeExpressionStatementNode(factory.makeAssignmentNode(
					factory.makeVariableAccessNode(null, factory.makeIdentifierNode("ret")),
					AssignmentOperator.PLUS_ASSIGNMENT, factory.makeBinaryExpressionNode(
							factory.makeStringLiteralNode((first ? "" : ", ") + fieldName + "="), toStringValueNode,
							BinaryOperator.PLUS))));
		...
		statements.add(factory.makeExpressionStatementNode(factory.makeAssignmentNode(
				factory.makeVariableAccessNode(null, factory.makeIdentifierNode("ret")),
				AssignmentOperator.PLUS_ASSIGNMENT, factory.makeStringLiteralNode("]"))));

		// return ret;
		statements.add(factory.makeReturnNode(factory.makeVariableAccessNode(null, factory.makeIdentifierNode("ret"))));
		...
		return factory.makeMethodDeclarationNode(factory.makeBlockStatementListNode(statements),
				factory.makeMethodModifiersNode(AccessModifier.PUBLIC), factory.makeIdentifierNode("toString"),
				factory.makeVariableListNode(),
				factory.makeUnparameterizedTypeNode(factory.makeSimpleNameNode(factory.makeIdentifierNode("String"))),
				factory.makeJavadocNode("Implementation of toString.\n"
