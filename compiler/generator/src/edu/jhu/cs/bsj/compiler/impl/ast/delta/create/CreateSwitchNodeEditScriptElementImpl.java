...
public class CreateSwitchNodeEditScriptElementImpl extends AbstractCreateEditScriptElementImpl
{
    ...
    public CreateSwitchNodeEditScriptElementImpl(
            int metaprogramId,
            long targetId,
            Long expression,
            Long cases,
    ...
    public void apply(PatchState patchState)
    {
        ...
        final CaseListNode casesNode = 
                (CaseListNode)patchState.getNode(this.cases);
        final NodeUnion<? extends CaseListNode> cases;
        {
            final NodeUnion<? extends CaseListNode> union;
            if (casesNode instanceof SpliceNode)
            {
                union = new SpliceNodeUnion<CaseListNode>((SpliceNode)casesNode);
            } else 
            {
                union = new NormalNodeUnion<CaseListNode>((CaseListNode)casesNode);
            }
            cases = union;
        }
        ...
        SwitchNode createdNode = patchState.getNodeFactory().makeSwitchNodeWithUnions(expression,
                cases,
                metaAnnotations,
                startLocation,
                stopLocation);
    ...
    @Override
    public Class<SwitchNode> getCreateType()
    {
    ...
    {
        return "[" + getMetaprogramId() + "]+#" + getTargetId() + ":SwitchNode(" + "expression=" + "#" + expression + "," + "cases=" + "#" + cases + "," + "metaAnnotations=" + "#" + metaAnnotations + "," + "startLocation=" + "{" + startLocation + "}" + "," + "stopLocation=" + "{" + stopLocation + "}" + ")";
    }
    
    public EditScriptElement translate(TranslationState translationState) throws IllegalArgumentException
    {
        translationState.getInstantiations().add(getTargetId());
        return new CreateSwitchNodeEditScriptElementImpl(
                getMetaprogramId(),
                getTargetId(),
                translateUid(translationState, this.expression),
                translateUid(translationState, this.cases),
