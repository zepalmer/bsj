...
import edu.jhu.cs.bsj.compiler.impl.ast.delta.property.ImportOnDemandNodeSetNamePropertyEditScriptElementImpl;
...
public class ImportOnDemandNodeImpl extends NodeImpl implements ImportOnDemandNode
{
     ...
     */
    private Set<ImportOnDemandNodeProperties> populatedProperties;
    ...
    /** Proxy constructor. */
    public ImportOnDemandNodeImpl(BsjNodeManager manager, BsjNodeProxyFactory proxyFactory, ImportOnDemandNode backingNode)
    {
        super(manager, proxyFactory, backingNode);
        this.populatedProperties = EnumSet.noneOf(ImportOnDemandNodeProperties.class);
    }
    
    /** Retrieves this node's backing node (if one exists). */
    protected ImportOnDemandNode getBackingNode()
    {
        return (ImportOnDemandNode)super.getBackingNode();
    }
    ...
    private void checkNameWrapped()
    {
        if (this.populatedProperties == null || this.populatedProperties.contains(
                ImportOnDemandNodeProperties.NAME))
            return;
        this.populatedProperties.add(ImportOnDemandNodeProperties.NAME);
    ...
    public void setUnionForName(NodeUnion<? extends NameNode> name)
    {
        ...
        if (this.getManager().isRecordingEdits())
            super.recordEdit(new ImportOnDemandNodeSetNamePropertyEditScriptElementImpl(this.getManager().getCurrentMetaprogramId(), this.getUid(), name.getNodeValue() == null ? null : name.getNodeValue().getUid()));
    }
    ...
    {
        return operation.executeImportOnDemandNode(this, p1, p2);
    }
    ...
    public ImportOnDemandNode deepCopy(BsjNodeFactory factory)
    {
        ...
        return factory.makeImportOnDemandNodeWithUnions(
                nameCopy,
                getStartLocation(),
                getStopLocation());
    }
