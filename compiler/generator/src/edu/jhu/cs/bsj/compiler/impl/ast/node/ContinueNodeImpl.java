...
import edu.jhu.cs.bsj.compiler.impl.ast.delta.property.ContinueNodeSetLabelPropertyEditScriptElementImpl;
import edu.jhu.cs.bsj.compiler.impl.ast.delta.property.ContinueNodeSetMetaAnnotationsPropertyEditScriptElementImpl;
import edu.jhu.cs.bsj.compiler.impl.ast.properties.ContinueNodeProperties;
...
public class ContinueNodeImpl extends NodeImpl implements ContinueNode
{
     ...
     */
    private Set<ContinueNodeProperties> populatedProperties;
    ...
    /** Proxy constructor. */
    public ContinueNodeImpl(BsjNodeManager manager, BsjNodeProxyFactory proxyFactory, ContinueNode backingNode)
    {
        super(manager, proxyFactory, backingNode);
        this.populatedProperties = EnumSet.noneOf(ContinueNodeProperties.class);
    }
    
    /** Retrieves this node's backing node (if one exists). */
    protected ContinueNode getBackingNode()
    {
        return (ContinueNode)super.getBackingNode();
    }
    ...
    private void checkLabelWrapped()
    {
        if (this.populatedProperties == null || this.populatedProperties.contains(
                ContinueNodeProperties.LABEL))
            return;
        this.populatedProperties.add(ContinueNodeProperties.LABEL);
    ...
    {
        if (this.populatedProperties == null || this.populatedProperties.contains(
                ContinueNodeProperties.META_ANNOTATIONS))
            return;
        this.populatedProperties.add(ContinueNodeProperties.META_ANNOTATIONS);
    ...
    /**
     * Gets the continue label.  This property's value is assumed to be a normal node.
    ...
    public void setUnionForLabel(NodeUnion<? extends IdentifierNode> label)
    {
        ...
        if (this.getManager().isRecordingEdits())
            super.recordEdit(new ContinueNodeSetLabelPropertyEditScriptElementImpl(this.getManager().getCurrentMetaprogramId(), this.getUid(), label.getNodeValue() == null ? null : label.getNodeValue().getUid()));
    }
    ...
    public void setUnionForMetaAnnotations(NodeUnion<? extends MetaAnnotationListNode> metaAnnotations)
    {
        ...
        if (this.getManager().isRecordingEdits())
            super.recordEdit(new ContinueNodeSetMetaAnnotationsPropertyEditScriptElementImpl(this.getManager().getCurrentMetaprogramId(), this.getUid(), metaAnnotations.getNodeValue() == null ? null : metaAnnotations.getNodeValue().getUid()));
    }
    ...
    public Map<String,Object> getChildMap()
    {
        ...
        map.put("label", getUnionForLabel());
    ...
    {
        return operation.executeContinueNode(this, p1, p2);
    }
    ...
    public ContinueNode deepCopy(BsjNodeFactory factory)
    {
        ...
        return factory.makeContinueNodeWithUnions(
                labelCopy,
                metaAnnotationsCopy,
                getStartLocation(),
                getStopLocation());
