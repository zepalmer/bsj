...
import edu.jhu.cs.bsj.compiler.impl.ast.delta.property.AnnotationBodyNodeSetMembersPropertyEditScriptElementImpl;
import edu.jhu.cs.bsj.compiler.impl.ast.properties.AnnotationBodyNodeProperties;
...
public class AnnotationBodyNodeImpl extends NodeImpl implements AnnotationBodyNode
{
     ...
     */
    private Set<AnnotationBodyNodeProperties> populatedProperties;
    ...
    /** Proxy constructor. */
    public AnnotationBodyNodeImpl(BsjNodeManager manager, BsjNodeProxyFactory proxyFactory, AnnotationBodyNode backingNode)
    {
        super(manager, proxyFactory, backingNode);
        this.populatedProperties = EnumSet.noneOf(AnnotationBodyNodeProperties.class);
    }
    
    /** Retrieves this node's backing node (if one exists). */
    protected AnnotationBodyNode getBackingNode()
    {
        return (AnnotationBodyNode)super.getBackingNode();
    }
    ...
    private void checkMembersWrapped()
    {
        if (this.populatedProperties == null || this.populatedProperties.contains(
                AnnotationBodyNodeProperties.MEMBERS))
            return;
        this.populatedProperties.add(AnnotationBodyNodeProperties.MEMBERS);
        NodeUnion<? extends AnnotationMemberListNode> union = this.getBackingNode().getUnionForMembers();
        switch (union.getType())
        {
            case NORMAL:
                union = this.getProxyFactory().makeNormalNodeUnion(
                        this.getProxyFactory().makeAnnotationMemberListNode(union.getNormalNode()));
                break;
            case SPLICE:
    ...
    {
        checkMembersWrapped();
        this.setUnionForMembers(new NormalNodeUnion<AnnotationMemberListNode>(members));
    }
    ...
    public void setUnionForMembers(NodeUnion<? extends AnnotationMemberListNode> members)
    {
        ...
        if (this.getManager().isRecordingEdits())
            super.recordEdit(new AnnotationBodyNodeSetMembersPropertyEditScriptElementImpl(this.getManager().getCurrentMetaprogramId(), this.getUid(), members.getNodeValue() == null ? null : members.getNodeValue().getUid()));
    }
    
    private void doSetMembers(NodeUnion<? extends AnnotationMemberListNode> members)
    {
        if (members == null)
        {
