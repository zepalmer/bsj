...
public class CreateEnumDeclarationNodeEditScriptElementImpl extends AbstractCreateEditScriptElementImpl
{
    ...
    public CreateEnumDeclarationNodeEditScriptElementImpl(
            int metaprogramId,
            long targetId,
            Long modifiers,
    ...
    {
        final EnumModifiersNode modifiersNode = 
                (EnumModifiersNode)patchState.getNode(this.modifiers);
        final NodeUnion<? extends EnumModifiersNode> modifiers;
        {
            final NodeUnion<? extends EnumModifiersNode> union;
            if (modifiersNode instanceof SpliceNode)
            {
                union = new SpliceNodeUnion<EnumModifiersNode>((SpliceNode)modifiersNode);
            } else 
            {
                union = new NormalNodeUnion<EnumModifiersNode>((EnumModifiersNode)modifiersNode);
            }
            modifiers = union;
        }
        ...
        final EnumBodyNode bodyNode = 
                (EnumBodyNode)patchState.getNode(this.body);
        final NodeUnion<? extends EnumBodyNode> body;
        {
            final NodeUnion<? extends EnumBodyNode> union;
            if (bodyNode instanceof SpliceNode)
            {
                union = new SpliceNodeUnion<EnumBodyNode>((SpliceNode)bodyNode);
            } else 
            {
                union = new NormalNodeUnion<EnumBodyNode>((EnumBodyNode)bodyNode);
            }
            body = union;
        }
        ...
        EnumDeclarationNode createdNode = patchState.getNodeFactory().makeEnumDeclarationNodeWithUnions(modifiers,
                implementsClause,
                body,
                identifier,
                javadoc,
    ...
    @Override
    public Class<EnumDeclarationNode> getCreateType()
    {
        return EnumDeclarationNode.class;
    }
    
    @Override
    public String toString()
    {
        return "[" + getMetaprogramId() + "]+#" + getTargetId() + ":EnumDeclarationNode(" + "modifiers=" + "#" + modifiers + "," + "implementsClause=" + "#" + implementsClause + "," + "body=" + "#" + body + "," + "identifier=" + "#" + identifier + "," + "javadoc=" + "#" + javadoc + "," + "startLocation=" + "{" + startLocation + "}" + "," + "stopLocation=" + "{" + stopLocation + "}" + ")";
    }
    
    public EditScriptElement translate(TranslationState translationState) throws IllegalArgumentException
    {
        translationState.getInstantiations().add(getTargetId());
        return new CreateEnumDeclarationNodeEditScriptElementImpl(
                getMetaprogramId(),
                getTargetId(),
