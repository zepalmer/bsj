...
public class IdentifierListNodeImpl extends ListNodeImpl<IdentifierNode> implements IdentifierListNode
{
    ...
    protected Class<IdentifierNode> getChildrenElementType()
    {
    ...
    {
        return operation.executeIdentifierListNode(this, p1, p2);
    }
    ...
    {
        List<NodeUnion<? extends IdentifierNode>> childrenCopy = new ArrayList<NodeUnion<? extends IdentifierNode>>(getChildren().size());
        for (NodeUnion<? extends IdentifierNode> element : getUnionForChildren())
        {
            NodeUnion<? extends IdentifierNode> elementCopy;
        ...
        return factory.makeIdentifierListNodeWithUnions(
                childrenCopy,
                getStartLocation(),
                getStopLocation());
    }
    ...
    public boolean replace(Node before, Node after)
    {
        ...
        if (after instanceof IdentifierNode)
        {
            int index = getChildren().indexOf(before);
            if (index != -1)
                getChildren().set(index, (IdentifierNode)after);
        }
        return false;
    }
