...
public abstract class AbstractDualWriteConflictClosureRule extends AbstractBinaryKnowledgeClosureRule
{
	private Class<? extends BinaryRelativeKnowledge<?>> clazz;

	public <T extends BinaryRelativeKnowledge<?>> AbstractDualWriteConflictClosureRule(String description,
			Class<T> clazz)
	{
		super(description);
		this.clazz = clazz;
	...
	public <T extends Node> ListKnowledge<T> calculateClosure(ListKnowledge<T> termA, ListKnowledge<T> termB)
	{
		if (this.clazz.isInstance(termA) && this.clazz.isInstance(termB))
		{
			...
			if (castedA.getAnchor().equals(castedB.getAnchor()) && castedA.getRelative().isOrderDependent()
					&& castedB.getRelative().isOrderDependent())
			{
