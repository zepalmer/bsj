...
import edu.jhu.cs.bsj.compiler.ast.node.list.knowledge.InvariantKnowledge;
 ...
 */
public abstract class AbstractInvariantKnowledgeIsNewInvariantInClosureRule extends AbstractUnaryKnowledgeClosureRule
{
	/** The class of knowledge on which to operate. */
	private Class<? extends InvariantKnowledge<?>> clazz;

	public <T extends InvariantKnowledge<?>> AbstractInvariantKnowledgeIsNewInvariantInClosureRule(
		...
		if (this.clazz.isInstance(term))
		{
			...
			return new ContainmentInvariantKnowledgeImpl<T>(binaryRelativeKnowledge.getMetaprogramId(),
					binaryRelativeKnowledge.getMetaprogramLocation(), new RuleKnowledgeSourceImpl<T>(this,
							Collections.singleton(term)), binaryRelativeKnowledge.getRelative());
		}
		return null;
	}
}
