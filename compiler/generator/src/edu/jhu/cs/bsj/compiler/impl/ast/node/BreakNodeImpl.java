...
import edu.jhu.cs.bsj.compiler.impl.ast.delta.property.BreakNodeSetLabelPropertyEditScriptElementImpl;
import edu.jhu.cs.bsj.compiler.impl.ast.delta.property.BreakNodeSetMetaAnnotationsPropertyEditScriptElementImpl;
import edu.jhu.cs.bsj.compiler.impl.ast.properties.BreakNodeProperties;
...
public class BreakNodeImpl extends NodeImpl implements BreakNode
{
     ...
     */
    private Set<BreakNodeProperties> populatedProperties;
    ...
    /** Proxy constructor. */
    public BreakNodeImpl(BsjNodeManager manager, BsjNodeProxyFactory proxyFactory, BreakNode backingNode)
    {
        super(manager, proxyFactory, backingNode);
        this.populatedProperties = EnumSet.noneOf(BreakNodeProperties.class);
    }
    
    /** Retrieves this node's backing node (if one exists). */
    protected BreakNode getBackingNode()
    {
        return (BreakNode)super.getBackingNode();
    }
    ...
    private void checkLabelWrapped()
    {
        if (this.populatedProperties == null || this.populatedProperties.contains(
                BreakNodeProperties.LABEL))
            return;
        this.populatedProperties.add(BreakNodeProperties.LABEL);
    ...
    {
        if (this.populatedProperties == null || this.populatedProperties.contains(
                BreakNodeProperties.META_ANNOTATIONS))
            return;
        this.populatedProperties.add(BreakNodeProperties.META_ANNOTATIONS);
    ...
    public void setUnionForLabel(NodeUnion<? extends IdentifierNode> label)
    {
        ...
        if (this.getManager().isRecordingEdits())
            super.recordEdit(new BreakNodeSetLabelPropertyEditScriptElementImpl(this.getManager().getCurrentMetaprogramId(), this.getUid(), label.getNodeValue() == null ? null : label.getNodeValue().getUid()));
    }
    ...
    public void setUnionForMetaAnnotations(NodeUnion<? extends MetaAnnotationListNode> metaAnnotations)
    {
        ...
        if (this.getManager().isRecordingEdits())
            super.recordEdit(new BreakNodeSetMetaAnnotationsPropertyEditScriptElementImpl(this.getManager().getCurrentMetaprogramId(), this.getUid(), metaAnnotations.getNodeValue() == null ? null : metaAnnotations.getNodeValue().getUid()));
    }
